...
- 텍스트 입력 포커스에서는 Escape·Undo·Clear뿐 아니라 Alt+Shift 도구 전환 global shortcut과 uIOhook fallback도 해제합니다. 기존 텍스트 편집 세션은 controller focus를 잃어도 격리를 유지하고, 편집 중 Undo/Redo/Clear와 새 overlay gesture로 문서를 바꾸지 못하게 하며, 세션 종료 뒤 판서 단축키를 복원합니다.
