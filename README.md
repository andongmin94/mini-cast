...
키보드 & 마우스 입력을 실시간으로 화면에 시각화(오버레이)하여 강의, 라이브 스트리밍, 발표, 튜토리얼 녹화에 도움을 주는 데스크톱 애플리케이션입니다. 다중 모니터 환경을 지원하며, 커서 하이라이트와 키 입력 표시를 세밀하게 커스터마이즈할 수 있습니다.

> 현재 애플리케이션 버전: **v0.1.2**  
> (문서 사이트 초기 hero 표기 v0.0.0 은 업데이트 예정)
...
---
## 라이선스
현재 별도 라이선스 파일이 없습니다. 공개 배포 전 **LICENSE** 추가 권장 (예: MIT).
