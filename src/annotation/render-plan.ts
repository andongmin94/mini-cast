...
export function planCommittedRender(
  ...
  let prefix = 0;
  while (
    prefix < previous.strokes.length &&
    prefix < next.strokes.length &&
    previous.strokes[prefix] === next.strokes[prefix]
  )
    prefix += 1;
  if (prefix === previous.strokes.length)
    return {
      kind: prefix === next.strokes.length ? "none" : "append",
      clear: null,
  ...
  if (left >= right || top >= bottom)
    return { kind: "none", clear: null, strokes: [] };
  ...
  return {
    kind: "dirty",
    clear,
    strokes: next.strokes.filter((stroke) =>
