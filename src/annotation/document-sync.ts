...
import { isAnnotationStroke, type AnnotationDocumentSnapshot, type AnnotationStroke } from "./history.js";
...
export interface AnnotationDelta {
  ...
  readonly inserted: readonly { readonly index: number; readonly stroke: AnnotationStroke }[];
}

export type AnnotationDocumentUpdate =
  | { readonly kind: "snapshot"; readonly document: AnnotationDocumentSnapshot }
  | AnnotationDelta
  | { readonly kind: "revision"; readonly displayId: number; readonly revision: number };

export type AnnotationMutationResult =
  | { accepted: true; update: AnnotationDocumentUpdate }
  | { accepted: false; reason: AnnotationFailureReason; update: AnnotationDocumentUpdate | null };
...
export function createAnnotationUpdate(
  ...
  if (!previous || previous.displayId !== next.displayId ||
  ...
  const oldById = new Map(previous.strokes.map(stroke => [stroke.id, stroke]));
  const newById = new Map(next.strokes.map(stroke => [stroke.id, stroke]));
  const removedIds = previous.strokes
    .filter(stroke => newById.get(stroke.id) !== stroke).map(stroke => stroke.id);
  ...
  const retained = previous.strokes.filter(stroke => newById.get(stroke.id) === stroke);
  let cursor = 0;
  for (const stroke of next.strokes) {
    if (oldById.get(stroke.id) === stroke && retained[cursor++] !== stroke)
      return { kind: "snapshot", document: next };
  }
  return { kind: "delta", displayId: next.displayId,
    baseRevision: previous.revision, revision: next.revision, removedIds, inserted };
}
...
export function reduceAnnotationUpdate(
  ...
  const id = update.kind === "snapshot" ? update.document.displayId : update.displayId;
  const revision = update.kind === "snapshot" ? update.document.revision : update.revision;
  if (displayId === null || id !== displayId) return { kind: "ignore" };
  if (!Number.isSafeInteger(revision) || revision < 0) return { kind: "resync" };
  ...
  if (current && revision <= current.revision) return { kind: "ignore" };
  if (update.kind === "revision" || !current ||
      !Number.isSafeInteger(update.baseRevision) ||
      update.baseRevision !== current.revision || revision <= update.baseRevision)
    return { kind: "resync" };

  const removed = new Set(update.removedIds);
  const currentIds = new Set(current.strokes.map(stroke => stroke.id));
  if (removed.size !== update.removedIds.length ||
      update.removedIds.some(id => !currentIds.has(id))) return { kind: "resync" };
  ...
  for (const { index, stroke } of update.inserted) {
    if (!Number.isSafeInteger(index) || index <= previousIndex || index >= finalLength ||
        !isAnnotationStroke(stroke) || ids.has(stroke.id)) return { kind: "resync" };
    ids.add(stroke.id);
    previousIndex = index;
  }
  ...
  return { kind: "adopt", document: {
    displayId, revision, viewport: current.viewport, strokes,
  } };
}
...
export class AnnotationReplica {
  ...
  async receive(update: AnnotationDocumentUpdate): Promise<AnnotationDocumentSnapshot | null> {
    const generation = this.generation;
    const decision = reduceAnnotationUpdate(this.current, this.displayId, update);
    if (decision.kind === "adopt") this.adopt(decision.document);
    if (decision.kind !== "resync") return this.current;

    const requiredRevision = update.kind === "snapshot" ? update.document.revision : update.revision;
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      if (!this.recovery) {
        const request = Promise.resolve().then(() => this.fetchSnapshot()).then(snapshot => {
          if (generation !== this.generation) return;
          const result = reduceAnnotationUpdate(this.current, this.displayId,
        ...
        }).catch(() => { /* The awaiting caller owns the visible failure. */ });
      }
      await this.recovery;
      if (generation !== this.generation) return null;
      if (this.current && this.current.revision >= requiredRevision) return this.current;
    }
    throw new Error("Annotation resynchronization did not reach the required revision");
  }
}
