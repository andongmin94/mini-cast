...
export async function verifyDirtyCanvasRendering(contents: WebContents) {
  ...
  return await contents.executeJavaScript(`(() => {
    ...
    for (const ratio of ratios) {
      ...
      const makeStroke = (id) => {
        const highlighter = random() % 2 === 0;
        return { id, tool: highlighter ? 'highlighter' : 'pen', opacity: highlighter ? 0.35 : 1,
          color: ['#FF0000', '#007AFF', '#FFD60A'][random() % 3], width: 1 + (random() % 160) / 10,
      ...
      const compare = (label, force = false) => {
        ...
        for (let i = 0; i < actual.length; i++) if (actual[i] !== expected[i])
          throw new Error('Dirty Canvas differs from full reference: ' + JSON.stringify({ ratio, label, channel: i, actual: actual[i], expected: expected[i], kind: plan.kind, clear: plan.clear }));
        comparisons++;
        previous = next;
      };
      compare('initial');
      for (let i = 0; i < 80; i++) {
        const saved = strokes;
        if (i % 5 === 0 && strokes.length) strokes = strokes.filter((_, j) => j !== random() % saved.length);
    ...
    if (!kinds.dirty || !kinds.append || !kinds.none) throw new Error('Canvas scenarios did not cover all plans');
    return { success: true, exactPixelComparisons: comparisons, ratios, kinds };
  })()`);
}
