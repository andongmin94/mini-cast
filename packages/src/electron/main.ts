...
import { createWindow, getMainWindow, createOverlayWindows, getOverlayWindows } from './window.js';
import { captureMouseEvents, captureKeyboardEvents, getConnectedDisplays } from './func.js';
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          createOverlayWindows(port, __dirname, currentSettings); // 오버레이 윈도우 생성
          captureMouseEvents(getOverlayWindows);
          captureKeyboardEvents(getOverlayWindows);
          createTray(getMainWindow, __dirname); // 트레이 생성
          setupIpcHandlers(getMainWindow, getOverlayWindows, () => store, currentSettings); // IPC 핸들러 설정
          
          screen.on('display-added', () => {
          const displays = getConnectedDisplays();
          getMainWindow()?.webContents.send('displays-updated', displays);
          createOverlayWindows(port, __dirname, currentSettings);
        });
