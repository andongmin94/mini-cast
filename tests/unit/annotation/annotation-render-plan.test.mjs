...
const state = (elements, overrides = {}) => ({ displayId: 1, viewportWidth: 1920, viewportHeight: 1080, canvasWidth: 1920, canvasHeight: 1080, pixelRatio: 1, elements, ...overrides });

test('initial paint clears the whole backing store and append paints only new elements', () => {
  ...
  const appended = planCommittedRender(state([a]), state([a, b])); assert.equal(appended.kind, 'append'); assert.equal(appended.clear, null); assert.deepEqual(appended.elements, [b]);
  assert.equal(planCommittedRender(state([a]), state([a])).kind, 'none');
});

test('local deletion repaints only intersecting elements in their original compositing order', () => {
  ...
  assert.equal(plan.kind, 'dirty'); assert.deepEqual(plan.elements, [b]);
...
test('Undo insertion beneath existing ink redraws the whole intersecting stack only once', () => {
  ...
  assert.equal(plan.kind, 'dirty'); assert.deepEqual(plan.elements, [a, b]);
});

test('reordering retained translucent elements invalidates their composited bounds', () => {
  ...
  assert.equal(plan.kind, 'dirty'); assert.deepEqual(plan.elements, [b, a]);
});
...
test('same IDs with replaced geometry or style do not reuse stale pixels', () => {
  ...
  assert.deepEqual(plan.elements, [moved]);
});
...
test('dirty rectangles are device-pixel aligned and clamped at fractional DPI and screen edges', () => {
  ...
  for (const ratio of [1, 1.25, 1.5, 2, 2.5]) {
    const s = state([a], { pixelRatio: ratio }); const plan = planCommittedRender(s, { ...s, elements: [] });
...
test('Clear clears prior ink bounds and an offscreen deletion does not repaint', () => {
  ...
  assert.equal(plan.kind, 'dirty'); assert.deepEqual(plan.elements, []);
...
test('a 1000-stroke local deletion avoids drawing unrelated geometry', () => {
  const elements = Array.from({ length: 1000 }, (_, i) => stroke(String(i), (i % 40) * 40, Math.floor(i / 40) * 35));
  const plan = planCommittedRender(state(elements), state(elements.slice(1)));
  assert.equal(plan.kind, 'dirty'); assert.ok(plan.elements.length < 5);
  console.log('DIRTY_REGION_WORK', JSON.stringify({ documentStrokes: elements.length, drawnStrokes: plan.elements.length, clearedPixels: plan.clear.width * plan.clear.height, fullPixels: 1920 * 1080 }));
});
