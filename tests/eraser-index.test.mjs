...
import { prepareEraserStroke, eraserSweepHitsPreparedStroke } from "../dist/annotation/eraser-index.js";

const stroke = (points, width = 4, id = "s") => ({ id, tool: "pen", color: "#000000", width, opacity: 1, points });
const hits = (points, start, end, radius, width = 4) => eraserSweepHitsPreparedStroke(start, end, prepareEraserStroke(stroke(points, width)), radius);
const counters = () => ({ strokeBoundsTests: 0, blockBoundsTests: 0, segmentTests: 0 });
...
test("wide-stroke and negative-coordinate queries agree with exact reference geometry", () => {
  const points = [{ x: -500, y: -400 }, { x: -100, y: -300 }, { x: 200, y: -800 }];
  ...
  for (let x = -600; x <= 300; x += 17) {
    ...
    for (const radius of [-1, 0, 14, 40]) {
      assert.equal(eraserSweepHitsPreparedStroke(a, b, prepared, radius), eraserSweepHitsStroke(a, b, source, radius));
    }
  }
});
...
test("seeded randomized sweeps exactly match the original exhaustive kernel", () => {
  ...
  for (let trial = 0; trial < 3000; trial += 1) {
    ...
    const points = Array.from({ length: count }, (_, i) => i % 11 === 0 ? { ...origin } : ({ x: origin.x + random() * 60, y: origin.y + random() * 60 }));
    const source = stroke(points, 0.5 + random() * 127.5);
    const prepared = prepareEraserStroke(source);
    const a = trial % 3 === 0 ? { ...origin } : { x: coordinate(), y: coordinate() };
    const b = trial % 5 === 0 ? { ...a } : { x: coordinate(), y: coordinate() };
    const radius = random() * 40;
    assert.equal(eraserSweepHitsPreparedStroke(a, b, prepared, radius), eraserSweepHitsStroke(a, b, source, radius), `trial=${trial}`);
  }
});

test("a local query avoids almost all exact segment tests in a 128k-point document", () => {
  const sources = Array.from({ length: 1000 }, (_, row) => stroke(Array.from({ length: 128 }, (_, x) => ({ x, y: row * 4 })), 2, `s-${row}`));
  const a = { x: 64, y: 2000 };
  const stats = counters();
  const actual = sources.map(prepareEraserStroke).filter(p => eraserSweepHitsPreparedStroke(a, a, p, 1, stats)).map(p => p.stroke.id);
  const expected = sources.filter(s => eraserSweepHitsStroke(a, a, s, 1)).map(s => s.id);
  ...
  console.log("Eraser local-query work:", JSON.stringify({ documentPoints: 128000, exhaustiveSegmentUpperBound: 127000, ...stats }));
});
