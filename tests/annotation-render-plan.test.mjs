...
test('painting clips and clears in device pixels, then restores the context even on draw failure',()=>{
  const calls=[];const context={save(){calls.push('save');},restore(){calls.push('restore');},setTransform(...x){calls.push(['transform',...x]);},beginPath(){calls.push('path');},rect(...x){calls.push(['rect',...x]);},clip(){calls.push('clip');},clearRect(...x){calls.push(['clear',...x]);},moveTo(){throw new Error('draw failure');},lineTo(){},stroke(){}};
  const a=stroke('a'), b=stroke('b');assert.throws(()=>paintCommittedAnnotations(context,state([a,b]),state([b])),/draw failure/);
  assert.ok(calls.indexOf('clip')<calls.findIndex(x=>Array.isArray(x)&&x[0]==='clear'));
