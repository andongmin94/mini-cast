"""Refine temporary-tool viewport cleanup and verify real post-cancel click routing.
This runs only in the opt-in Windows job and is removed after all gates succeed.
"""
from pathlib import Path
import json
import subprocess

# The historical runner does not stop on an earlier Python process failure.
# Fail closed before any preparation step can fail, and restore check only at the end.
guard=json.loads(Path('package.json').read_text(encoding='utf-8'))
guard['scripts']['check']='node -e "throw new Error(\'Transient viewport refinement incomplete\')"'
Path('package.json').write_text(json.dumps(guard,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
raw=subprocess.check_output(['git','cat-file','-p','HEAD']).decode('utf-8')
BASE=next(line.split()[1] for line in raw.splitlines() if line.startswith('parent '))
...
replace(smoke,'  await draw();\n  const loaded=new Promise<void>', '''  // Real Chromium zoom changes CSS coordinates and DPR together. This is not a physical-monitor test.
  await draw();
...
files['docs/CHANGELOG.md']=source('docs/CHANGELOG.md')+'\n- 실제 Chromium 배율 변경과 실제 클릭 차단·Escape 후 복귀를 검증했습니다. 포인터가 없는 화면의 빈 임시 레이어는 반복해서 다시 그리지 않습니다. 물리 모니터 DPI 검증은 아닙니다.\n'
for name,content in files.items(): write(name,content)
write('package.json',json.dumps(package,ensure_ascii=False,indent=2)+'\n')
print('TRANSIENT_VIEWPORT_REFINEMENT_COMPLETE version=0.11.0; native routing and Chromium zoom diagnostics are mandatory')
